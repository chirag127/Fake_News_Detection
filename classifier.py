...
def show_most_informative_features(model, vect, clf, text=None, n=50):
    ...
    if not hasattr(classifier, 'coef_'):
        raise TypeError(
            f"Cannot compute most informative features on {classifier.__class__.__name__}."
        )


    tvec = classifier.coef_ if text is None else model.transform([text]).toarray()
    ...
    if text is not None:
        output.extend((f'"{text}"', f"Classified as: {model.predict([text])}", ""))
